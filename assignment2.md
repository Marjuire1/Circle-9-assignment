...
- REACT is a Javascript framework developed at facebook in 2011. It is used for building fast and active user interfaces(UI).
- React provides us with a set of tools and structure for building out these user interfaces which makes the process faster and easier.

---
# What can you do with REACT
Did you know that with REACT it is very common to build single page application?
Single template applications work with one template and are simply updating all the components within the DOM. To clarify, it just means that single templates are being used to modify all contents within it.They are not one page within the application.

---
# Components
Components are the building blocks of  REACT applicstion. They are also the reusable pieces of UI that can manage their own stage and logic. They make up thevisual layer of the application and enables us split our UI into independent reusable pieces.

---
Furthermore, components can be nested as deep as you want. A component can hold another component and that component can hold another component. Components are usually built in standard-alones.
There are two main types of components which are functional components and class components. Function-based components are commonly used in REACT unlike clas components.

---
It is noteworthy, that components return back some HTML. For example,
...
---
# KEY POINTS
- REACT Events are how you handle user interactions in a REACT application. Things like clicks, form submission, key process, etc. REACT wraps synthetic DOM events with its own synthetic event system for better cross-browser compatibility. Event names use camelCase (e.g onClick instead of onclick).
- Event handlers are passed as functions and not as strings.
- Conditional rendering in REACT means displayind different UI elements based on certain conditions (like, if, else, or switch in regular Javascript).
- REACT Hooks are functions that let you use REACT features in functional components- which previously only class components could do.
